...
def create_categorical_dataframes(feature_file, target_file):
    ...
    current_prepper.fit_transform_quantile_col('image_nfavs', 10)
    current_prepper.fit_transform_quantile_col('image_ncomments', 10)
    current_prepper.fit_transform_quantile_col('image_nsets', 10)
    current_prepper.fit_transform_quantile_col('image_npools', 10)
...
def main(destination_folder):
    ...
    for feature_file, target_file in zip(feature_file_list, target_file_list):
        (current_prepper,
            search_term) = create_categorical_dataframes(feature_file,
                                                         target_file)
        current_prepper.scale_feature_data()
    ...
    all_data_prepper.scale_feature_data()
